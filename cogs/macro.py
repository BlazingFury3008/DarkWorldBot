...
class Macro(commands.Cog):
    ...
    async def create_macro(self, interaction: discord.Interaction, character: str, name: str, macro_str: str):
        ...
        try:
            ...
            if not valid:
                await interaction.response.send_message(f"❌ Invalid macro: {error}", ephemeral=True)
                return
            ...
            if name in char.macros:
                await interaction.response.send_message(f"⚠️ A macro named `{name}` already exists. Use `/macro update` to modify it.", ephemeral=True)
                return

            char.macros[name] = macro_str
            char.save()

            await interaction.response.send_message(f"✅ Macro `{name}` created for **{character}**.", ephemeral=True)

        except Exception as e:
            logger.error(f"Error creating macro: {e}")
            await interaction.response.send_message(f"⚠️ Error creating macro: {e}", ephemeral=True)
    ...
    async def update_macro(self, interaction: discord.Interaction, character: str, name: str, macro_str: str):
        ...
        try:
            ...
            if not hasattr(char, "macros") or name not in char.macros:
                await interaction.response.send_message(f"⚠️ No existing macro named `{name}` found.", ephemeral=True)
                return

            char.macros[name] = macro_str
            char.save()

            await interaction.response.send_message(f"✏️ Macro `{name}` updated for **{character}**.", ephemeral=True)

        except Exception as e:
            logger.error(f"Error updating macro: {e}")
            await interaction.response.send_message(f"⚠️ Error updating macro: {e}", ephemeral=True)
    ...
    async def list_macros(self, interaction: discord.Interaction, character: str):
        ...
        try:
            ...
            if not macros:
                await interaction.response.send_message(f"ℹ️ **{character}** has no saved macros.", ephemeral=True)
                return
            ...
            for macro_name, macro_expr in macros.items():
                ...
                if total == -1:
                    value_str = "⚠️ Invalid or unknown traits"
                else:
            ...
            await interaction.response.send_message(f"⚠️ Error listing macros: {e}", ephemeral=True)
    ...
    async def delete_macro(self, interaction: discord.Interaction, character: str, name: str):
        ...
        try:
            char = Character.load_by_name(name=character, user_id=user_id)
            if not hasattr(char, "macros") or name not in char.macros:
                await interaction.response.send_message(f"⚠️ No macro named `{name}` found.", ephemeral=True)
                return

            del char.macros[name]
            char.save()

            await interaction.response.send_message(f"🗑️ Macro `{name}` deleted from **{character}**.", ephemeral=True)

        except Exception as e:
            logger.error(f"Error deleting macro: {e}")
            await interaction.response.send_message(f"⚠️ Error deleting macro: {e}", ephemeral=True)
